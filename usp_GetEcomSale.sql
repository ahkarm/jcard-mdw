...
/****** Object:  StoredProcedure [dbo].[usp_GetEcomSale]    Script Date: 11/5/2025 12:37:06 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_GetEcomSale]
AS
BEGIN
    SET NOCOUNT ON;

    DECLARE @LoopCount INT = FLOOR(RAND(CHECKSUM(NEWID())) * 3) + 1;  
    DECLARE @Counter INT = 1;

    BEGIN TRY
        ---------------------------------------------
        -- 1. Insert E-Commerce Sale Transactions
        ---------------------------------------------
        ...
        BEGIN
            ...
            (
                ...
                (SELECT CODESUBTYPE FROM D_CODETYPE WHERE CODETYPE = 'TXCODE' AND RECID IN (24)),
                'SCB',
                'SICOTHBKXXX',
                '010000',
                '-',
                (SELECT TOP 1 CODESUBTYPE FROM D_CODETYPE WHERE CODETYPE = 'MCC' ORDER BY NEWID()),
        ...
        BEGIN TRAN;
            ...
            SET 
                ...
                DESCS = CASE WHEN TXCODE = 'DPT_ECOM_SALE' THEN 'E-Commerce Sale' END,
                CHAR04 = CASE WHEN TXCODE = 'DPT_ECOM_SALE' THEN '{"tx_desc":"E-Commerce Sale"}' END
            WHERE TXCODE = 'DPT_ECOM_SALE'
              AND ACNO IS NULL;
        COMMIT TRAN;

        ---------------------------------------------
        -- 3. Update TX Amount & Fee
        ---------------------------------------------
        BEGIN TRAN;
            UPDATE D_EBSTM
            SET 
        ...
        BEGIN TRAN;
            ...
            SELECT 
                ...
                TXDT, '180'
            FROM D_EBSTM
            WHERE TXCODE = 'DPT_ECOM_SALE' AND CHAR06 IS NULL;
        COMMIT TRAN;
