...
/****** Object:  StoredProcedure [dbo].[usp_GetPOSSale]    Script Date: 11/5/2025 12:37:51 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_GetPOSSale]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
        ...
        INSERT INTO D_EBSTM
        (
            TXTIME, TXCODE, BANKCODE, BICCODE, PCODE, COMCODE, MERCODE, AMT,
            CCYCODE, STATUS, CHAR01, CHAR02, CHAR03, BILLINFO, TRANSID, ISCSHBACK
        )
        VALUES
        (
            ...
            (SELECT TOP 1 CODESUBTYPE 
             FROM D_CODETYPE 
             WHERE CODETYPE = 'TXCODE' AND RECID IN (22,23) 
             ORDER BY NEWID()),
            'SCB',
            'SICOTHBKXXX',
            '010000',
    ...
    SET 
        ...
        DESCS = CASE 
                    WHEN TXCODE = 'DPT_POS_SALE_OFF' THEN 'POS Sale Off-US'
                    WHEN TXCODE = 'DPT_POS_SALE_OUS' THEN 'POS Sale On-US'
                END,
        CHAR04 = CASE 
                    WHEN TXCODE = 'DPT_POS_SALE_OFF' THEN '{"tx_desc":"POS Sale Off-US"}'
                    WHEN TXCODE = 'DPT_POS_SALE_OUS' THEN '{"tx_desc":"POS Sale On-US"}'
                END
    WHERE TXCODE IN ('DPT_POS_SALE_OFF', 'DPT_POS_SALE_OUS') 
          AND ACNO IS NULL;
    COMMIT TRAN;
    ...
    SET 
        NUM19 = AMT,
        CHAR05 = CASE 
                    WHEN TXCODE IN ('DPT_POS_SALE_OFF') THEN '{"fee_code":"42"}'
                    ELSE '{"fee_code":"-"}'
                 END,
        NUM01 = CASE 
                    WHEN TXCODE IN ('DPT_POS_SALE_OFF') THEN 5.00
                    ELSE 0
                 END
    WHERE TXCODE IN ('DPT_POS_SALE_OFF', 'DPT_POS_SALE_OUS')
          AND CHAR05 IS NULL;
    ...
    SET 
        ...
        ERRCODE = CASE 
                    WHEN STATUS = 'C' THEN 'Transaction Successfully!' 
                    ELSE 'Transaction Failed at JCard Middleware Side!' 
                  END,
        RESPCD = CASE 
                    WHEN STATUS = 'C' THEN 'Transaction Successfully!' 
                    ELSE 'Transaction Failed at JCard Middleware Side!' 
                 END,
        NUM20 = CASE 
                    WHEN STATUS = 'C' THEN NUM01 + NUM19 
                    ELSE 0 
                 END,
