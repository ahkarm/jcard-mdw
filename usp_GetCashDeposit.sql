...
/****** Object:  StoredProcedure [dbo].[usp_GetCashDeposit]    Script Date: 11/5/2025 12:36:03 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_GetCashDeposit]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        -------------------------------
        -- 1. Insert Cash Deposit Transaction
        -------------------------------
        ...
        BEGIN
            ...
            VALUES(
                ...
                (SELECT CODESUBTYPE FROM D_CODETYPE WHERE CODETYPE = 'TXCODE' AND RECID IN (17)),
                'SCB',
                'SICOTHBKXXX',
                '020000',
                '11011',
        ...
        BEGIN TRAN;
            ...
            SET 
                ...
                DESCS = 'Cash Deposit via ATM',
                CHAR04 = '{"tx_desc":"Cash Deposit via ATM"}'
            WHERE TXCODE = 'DPT_CSH_DPT_ATM'
              AND ACNO IS NULL;
        COMMIT TRAN;
