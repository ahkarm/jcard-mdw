...
/****** Object:  StoredProcedure [dbo].[usp_GetDebitCard]    Script Date: 11/5/2025 12:36:58 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_GetDebitCard]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        -------------------------------
        -- 1. Add Debit Card Info (Random Inserts)
        -------------------------------
        SET @LoopCount = FLOOR(RAND(CHECKSUM(NEWID())) * 5) + 1;
        SET @Counter = 1;
        ...
        BEGIN
            BEGIN TRAN;
                ...
                (
                    ...
                    'D',
                    (SELECT TOP 1 CODESUBTYPE FROM D_CODETYPE WHERE CODETYPE = 'DEBIT' ORDER BY NEWID()),
                    CONCAT(
                        CAST(ABS(CHECKSUM(NEWID())) % 900000 + 100000 AS INT),
                        CAST(ABS(CHECKSUM(NEWID())) % 900000 + 100000 AS INT)
                    ),
        ...
        BEGIN TRAN;
            ...
            WHERE CARDBIN IS NULL AND CARDTYPE = 'D';
        COMMIT TRAN;
        ...
        BEGIN TRAN;
            ...
            SET APP = 'BO',
                ...
                TARGET_USER_ID = '{"folder_id":"debit_card"}'
        ...
        BEGIN TRAN;
            ...
            SELECT 
                ...
                'EB' AS CHAR17
            FROM D_CARDLST
            WHERE CARDSTS = 'P' AND STATUS = 'W' AND CARDTYPE = 'D';
        COMMIT TRAN;

        -------------------------------
        -- 6. Update D_CARDLST to mark debit cards processed
        -------------------------------
        BEGIN TRAN;
            UPDATE D_CARDLST
            SET CARDSTS = 'N',
